...
{
    public partial class ChargeUCtrl : UserControl
    {
        public ChargeUCtrl(SerialPort c)
        {
            InitializeComponent();
            com = c;
            #region 定时器相关
        ...
        private void InvokeMethod(int count)
        {
            ...
            {
                button3.Text = "测试(" + count.ToString() + "s)";
                if (count == 3)
                {
                    timer.Stop();
                    //Thread.Sleep(3000);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("请选择一个测试结果");
                return;
            }
            //测试完成之后将结果写入数据库
            if (Pass.Checked)
            {
                op.updateTestResult(11, "pass");                
            }
            else
            {
                op.updateTestResult(11, "fail");                
            }
            button1.Text = "完成";
