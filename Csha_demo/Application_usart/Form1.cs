        ...
        {//发送数据
            ...
            {//如果串口开启
                if (textBox_send.Text.Trim() != "")//如果框内不为空则
                {
                    ...
                    //int num =  int.Parse(textBox_send.Text.Trim(), System.Globalization.NumberStyles.HexNumber);
                    //char[] inputs = textBox_send.Text.Trim().ToCharArray();
                    //serialPort1.Write(inputs, 0, inputs.Length);
                    //string datapacket = "f02e00010200ff";
                    byte[] array = HexStringToByteArray(textBox_send.Text.Trim());
